...
class CountryData
  ...
  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent = 'South America' ORDER BY gni ASC OFFSET 2 ROWS LIMIT 1;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.all_countries_not_in_europe
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent NOT IN ('Europe');")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.countries_not_in_asia_hide_gni_per_capita_and_population_density
    result = DatabaseConnection.query("SELECT name, population, continent FROM countries WHERE continent NOT IN ('Asia');")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
end
